...
| 真后端上的"六题可选"核对 | ⬜ 待环境：C 告知受控白名单已从 1 条扩到 6 条（`adapters/tasks/catalog.py` 的 `FIXED_TASK_IMAGES`，preset id 形如 `swe-gym-lite-mypy-15131`），向导应在**真后端**上用这 6 个 preset id 核对。浏览器的合成夹具用的是 `swe-gym-lite-example-*` 合成 id，只能证明向导的通用渲染，**不能代替真后端核对**；需 B 能访问带受控目录的真后端 |
