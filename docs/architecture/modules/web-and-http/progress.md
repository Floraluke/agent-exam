...
- **为放这份准备件做的位置选择**：本模块 `actions/` 目录现有 **10 个文件，已超过"每层文件夹不超过 8 个文件"的指标**，因此**没有往该目录再加第 11 个文件**，准备件放在 `docs/actions/`（与 D 的准备件同位置、同命名风格）。**顺带发现一个待你决定的事项**：`actions/` 的超标需要拆分口径——候选是按任务区间分两个子目录（`01–05` 任务态 / `06+` 记录态），或把已被取代的（如 `06-full-web-standalone-prototype.md`）移入 `actions/archive/`；拆分要移动既有文件并同步修正引用，等你确认再动。
...
- 细节见[行动记录 08](actions/08-real-web-detached-build.md)；被取代的[行动记录 06](actions/06-full-web-standalone-prototype.md)只作历史保留。
