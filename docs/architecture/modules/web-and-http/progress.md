...
| 缺口 | 现状与原因 |
|---|---|
| 对比矩阵在**多列**量级下的横向滚动手感 | ✅ **已补（2026-09-21）**：新增浏览器用例用接口批量建 **12 个批次**（六题×两配置），在 390px 下断言列头 13 个、容器 `scrollWidth > clientWidth`（**真的压出横向滚动**）且页面不溢出；截图 `runtime/tests/03-comparison-wide-390.png`。20 列以上仍未强制（夹具只有六题） |
| 任务 04 的"六题×三配置"里的**第三个配置** | 夹具只种 2 个配置；夹具文件已因超指标走例外，再加会更超，需先决定是否拆分 |
