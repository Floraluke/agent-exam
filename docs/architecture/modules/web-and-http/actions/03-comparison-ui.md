...
> 状态：**候选 / 待确认**（2026-09-21 起草）。入口已确认（侧栏新增「对比报告」，`view=reports`）；批次多选交互与两处展示决策待确认（见第 4 节）。
>
...
| 批次选择（多选，1–20） | 交互候选见第 4.1 节；本地保存所选 Job 列表，去重保序 | 选择来源为现有 `GET /api/v1/jobs`（如经列表选择） | 可见范围与列表相同；collaborator 只能看到自己的 Job | 边界：0 个禁用、20 个封顶、重复选择去重、协作者范围 |
...
1. **批次多选交互方式**（影响「批次选择」行）：从评测列表多选带入对比页，还是对比页内添加/移除 Job？
2. **失败后矩阵状态**：请求失败（404/503/网络）时，保留旧矩阵并提示，还是清空回空态？
3. **列汇总行的数值呈现**：`decided/total` 之外是否同时显示五档细分数字（当前草案写"五档计数 + 比值"，若觉得拥挤可只留比值）。
4. **限制/工具/网络差异**：草案只经列头进 Job 详情查看；若要求在矩阵页直接展示，需要后端给列补充字段，超出 §10.4 现状，须另行确认。
