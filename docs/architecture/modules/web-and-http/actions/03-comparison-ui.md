...
**⚠️ 落地前必须先确认的前置（重要）**：`HTTP_API.md` §10.4 的响应**目前不返回**这些指标（只有五档计数与 `decided`/`total`）。因此真要落地，需要**先在契约层给列补字段**（后端 + §10.4 + D 的报告语义），属超出本 Module 单独改动的范围，须另行确认——本原型只解决"设计成什么样"，不代表契约已支持。
