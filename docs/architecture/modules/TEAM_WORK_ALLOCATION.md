...
7. **分工不等于全部开工。** 01–08 当前仍是规划编号，未发布独立 issue；实施顺序仍按计划和用户当次安排。M1-14 已发布且在进行中，可继续补剩余验收。
...
| **01 可点击 HTML 原型** | 已规划、未发布 issue | **B** | B：三套布局、角色/状态/手机原型；A/C/D：审核权限、目录和状态文案，不改产品代码 | 现有页面/API事实和用户交互偏好 | 用户确认的页面清单、交互说明、桌面/手机截图；确认后停 |
| **02 两角色首页、列表、提交/审批** | 已规划、未发布 issue | **B** | B：工作台/向导/浏览器；A：会话/角色；C：选项；D：提交、审批、取消/恢复契约 | 01 选定原型；现有身份、目录和 Job Interface | 两角色真实 UI 闭环及兼容 HTTP；不改批次规模，完成后停 |
