...
> 状态：**准备性设计，未开工**。05 尚未发布独立 issue、未获实施授权。本文件不代表已实现、已授权或任何测试已运行。
>
...
本机（`D:\agent-exam`）按阶段 0 决定不安装 Docker，且 `framework/`、`runtime/`、固定镜像只在组长机器上。因此**设计的第一原则是让绝大多数断言落在不需要容器的一层**，把容器依赖压到最小。
...
1. **Token 上界、请求字段白名单、账本具体格式**——实现地图第 5 节末尾明示尚未验证。
...
4. **私有文件路径、格式与权限拒绝条件**——入口已由 Q13 确认，精确条件仍须在实施计划中明确。

## 6. 前置条件与风险

| 项 | 影响 | 现状 |
|---|---|---|
| 第一步必须在组长机器 | 本机无 Docker，拓扑实证与集成层全部无法在本机运行 | 需尽早与负责人对齐组长机器的可用时间窗口 |
