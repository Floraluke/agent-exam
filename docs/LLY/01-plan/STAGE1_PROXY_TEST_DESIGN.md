...
> 状态（2026-09-22 按实际进度同步）：**准备性设计；其中的用例归属已在 `tests/providers/{policy,contract,lifecycle,runtime}` 落地**。任务 05 已进 `main`，本机实施已获会话内实施开工同意；**集成层（负责人机器）部分仍等 T2**。本文件保留"权威验收要求 → 测试归属、断言与运行位置"的映射，不代表集成层测试已运行。
>
