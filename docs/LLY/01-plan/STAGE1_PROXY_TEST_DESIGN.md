...
| 项 | 影响 | 现状 |
|---|---|---|
| 第一步必须在组长机器（**2026-09-21 部分变更**） | 原为"本机无 Docker，拓扑实证与集成层全部无法在本机运行"。现本机已装 Docker，纯 Docker/Compose 层可搬本机；**固定 Harbor 集成层仍须负责人机器** | 本机 Docker 能否创建自定义网络未验证；两半拆分与执行授权待负责人确认 |
