...
- **T2 的状态（2026-09-22 更新）**：T1 已在负责人机器复测通过；Harbor 源码结论为"允许按服务绕过侧车附加"；**授权增补已给出**（见上方与本日条目），因此**不再缺授权，缺的是在负责人机器上按"最小 T2 形态"跑一次并回报**。
- 推送状态：本机与 `origin/lly/dev` 一致（上一条"领先 14 个提交"的说法已被本轮推送取代）。
- 仍等 B：受控提供方的响应呈现（枚举扩宽与如实读取记录）与 §10.2 受控码列入枚举。
