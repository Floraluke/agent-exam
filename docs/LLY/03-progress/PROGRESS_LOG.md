...
- **一处待用户拍板的架构选择**：受控假提供方在目录里用什么身份（是否新增一个只在该隔离拓扑内可解析的受控 provider 值），它决定库级 CHECK 与上游登记表怎么写，故 S8 主体等它。
- 已推送：`40a5f16`…`8b562c5` 均在 `origin/lly/dev`，未合并进 `main`。
- **下一步（E 侧）**：① 拍板后做 S8 主体（库级约束显式升级脚本 + 注册表校验 + 响应枚举与读取记录同批 + 端到端用例）；② S9 依赖 S8 登记侧成形，排其后；③ S2 字段名等固定 CLI 复核（负责人机器）；④ `service.py` 网络接线等 T2 结论，可先做鉴权、令牌生命周期与错误码映射；⑤ 本任务链条落地后**通知 B** 排期加“强制下一次响应出错”的夹具端点。
