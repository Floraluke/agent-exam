...
- CR-14：把 Web/HTTP 当前状态表更新到 `01feba4` 的后端、根入口和无参数
  Mypy 实测，并把 `fb8aadf` 运行段显式标为历史时点。
- CR-15：删除当前进展中的一份逐字重复段落，只保留单一记录。
- CR-16：删除 `parseArtifactPage` / `ArtifactPage` 传递性死代码；后端 artifact
  索引端点继续保留，HTTP 追踪表明确 Web 当前未接线，报告页实际从 Run report
  的 `artifact_links` 读取。
- CR-17：在当前进展文档纠正“本机无 Docker”的失效理由；已结束的 06/07
  准备行动保持原文，固定 Harbor、私有环境与正式运行身份仍是负责人环境约束。
- 验证：Web lint、TypeScript、隔离目录生产构建和系统 Chrome 45 项回归通过；
  未完成批次报告定向用例 `4 passed`；后端默认全量 `510 passed / 102 skipped`、
  分支覆盖率 86.38%；仓库根统一入口 `521 passed / 112 skipped`；Ruff、321
  文件格式检查、无参数 Mypy 177 个源文件通过；Markdown 相对链接与 Git 空白
  检查通过。
- 单文件 Pytest 的第一次运行虽有 4 条断言通过，但因单文件覆盖率 44.12% 低于
  全局 80% 门槛而失败；这次只作为检查方法失败保留。随后以 `--no-cov` 完成
  定向行为检查，并用默认全量 86.38% 覆盖率门禁关闭该限制。
- Next 构建在沙箱内先被用户配置 ACL 拒绝，沙箱外又遇到用户配置临时文件的
  跨设备重命名失败；设置 `NEXT_TELEMETRY_DISABLED=1` 后项目编译、类型检查和
  静态页面生成全部通过。隔离构建目录已验证绝对路径后精确删除，自动改写的
  Next 配置已恢复并复测。
