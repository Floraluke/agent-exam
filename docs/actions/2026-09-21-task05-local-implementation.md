...
- **`service.py` 与 S9 尚未实施**（S2 的渲染已实现、字段名待复核）；已完成前置验证、S3–S7、T1、S8（筛选接线 + 受控身份与登记）、S6 的受控文案映射片段、契约层的假 Responses 上游。**代理中所有能以纯逻辑表达的安全不变量均已落地并有测试**：私有文件可信、请求出站前拒绝、额度不超支、未知不记零、令牌不跨 Run、出站目标不由请求决定、重试与重定向结构上不可配。
