...
- 状态：Completed（首次身份实施与分层验证已收尾；随后评审发现的问题由[新的独立修复行动](2026-09-11-m1-identity-review-fixes.md)处理，不复用本记录承载第二次行动）。对应[任务 01](../../.scratch/m1-platform/issues/01-owner-login-recovery.md)，本次行动独立于已完成并本地提交为 `2290973` 的任务发布行动。
...
- 首次实施收尾时尚未运行正式 `code-review`。随后用户已批准以 `2290973` 为固定点、`b15f062` 为实现检查点的 Standards/Spec 双轴评审；发现两项 HTTP 契约问题和一项浏览器缺证，未把有问题的评审记为通过。用户确认修复后另开[修复行动](2026-09-11-m1-identity-review-fixes.md)，后续源码、文件树和最新验证由该记录维护。提交号与恢复边界见 [HANDOFF 第 5 节](../../HANDOFF.md#5-git环境与测试快照)；此前混合修改的权威文档保持在工作区，不整批混入本次提交。
- 文档收尾检查：链接检查发现两处新增引用错误（行动文件名漏月份、交接标题锚点不符），逐一核对修正后，最终 8 份变更文档、189 个本地链接和 47 个锚点全部通过；围栏/行尾空白检查通过。写入工具曾返回上下文不匹配，但核查发现首份文件已部分写入，故逐文件确认后只补写未生效部分，没有盲目重放整批补丁。71 个既有 M0 源码/测试/配置（排除本次明确更新的包清单/锁）与行动开始前哈希一致。
- 暂存范围预检发现 `apps/web/%USERPROFILE%/` 下的 npm 通知缓存，不是源码，和根目录此前的同名未跟踪路径均不读取、不暂存或删除。构建/运行缓存不进入检查点；后续可在独立维护范围中清理，不扩大本次身份工作。
- 检查点结果：先确认暂存区为空，再用明确路径列表加入 43 文件；实际暂存集合与允许集合相同，`git diff --cached --check` 通过。本地 commit 成功，随后暂存区为空；没有 amend、推送、修改 M0 源码或把历史混合文档一并提交。该检查点用于恢复和后续评审，不表示 M1 或任务 01 已全部验收。
