...
### 08 操作手册（D 侧，草稿；步骤与验收标准以 `plan.md` 第 10 节为准）

本节只写"D 怎么做"，不复制任务步骤与验收标准。

1. **冻结与提交（每配置一批）**：在 Web 向导按"选题→选配置→核对提交"提交；规模预设选 `continuous`（六题落在 1–20 内）。两个新配置各提交一批，等待 owner 批准。提交只返回待批 Job，不代表已开始执行。
2. **执行观察**：只读平台状态（Job/Run 状态与事件），不干预执行；零自动重试，失败保留原样、不为挑结果复跑。
3. **逐 Run 收集证据**：按上文记录模板填写（终态、确定性结果、用量与费用、基础设施失败码、证据引用、清理确认）；缺失与未知一律标注，不写 0。
4. **生成对比报告**：
   - 取得两批的批次报告（`GET /api/v1/reports/jobs/{job_id}`，或经 Web 详情页）；
   - 用本分支的 `matrix.py` 聚合成"题目 × 配置"矩阵，`matrix_markdown.py` 渲染成 Markdown 表；
   - 口径：分母 = 完整矩阵（含缺失）；缺失不写成 0、不计入"未通过"、不从分母扣除；覆盖率单独给出（"有结论 N / 总数 M"）。
   - 若届时 Web 已提供跨批次对比，直接使用界面结果并注明来源，避免两份口径。
5. **回归清单**：按 `.scratch/ui-catalog-providers/verification.md` 逐项执行（默认后端、Web 类型检查与生产构建、浏览器动线、隔离真实存储、代理与固定 Fork 回归），逐项记录"通过/失败/跳过/延期"及实际命令与输出。
6. **收口**：双轴（Standards/Spec）评审 → 修复 → 定向重验（必要时全量）；结论写回 08 行动文档并同步权威文档；M1-14 未关单时不得宣布整个 MVP 完成。
