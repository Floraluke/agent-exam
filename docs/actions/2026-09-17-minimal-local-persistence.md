...
状态：In progress / 等待用户取得 Free 许可证（2026-09-17；AIStor Free 修复版方向已确认，官方版本/许可核对已完成，目标保持 active，未完成）。P1 配置及 P3 Worker 命令控制切片已验证，但不是完整持久化交付。本行动从用户发布持续目标开始，独立于先前已完成的模块/单机规划行动。交付范围与成功标准以[行动指南](../architecture/modules/owner-host-runtime/ACTION_GUIDE.md)为准；P1–P5 尚未完成，不把基线测试当部署验收。
...
新增执行要求：每半小时检查 Codex 账户额度；若剩余 1–4%，先本地提交本目标安全检查点并报告停止。当前工具目录没有自动化或直接额度读取工具；通过[官方 App Server](https://learn.chatgpt.com/docs/app-server) 的 `account/rateLimits/read` 查询。沙箱内两次返回 `-32600`，第二次明确归类为认证不可用；经审批的原生接口查询成功。最新恢复后检查为 **2026-09-17 21:01:28（新加坡时间）**：`codex` 主窗口剩余 **25%**，`codex_bengalfox` 两窗口均 100%，未触发停工阈值；持续执行的下次检查最迟 **21:31**。历史 20:26 为 28%、20:53 为 26%；不以 goal token 数或另一限额池替代。只查询限额，不创建模型 turn、不打开/输出认证文件、不重置登录；辅助进程已退出。未建立后台自动化，暂停后的后台半小时检查不能保证。
...
P1 配置和 P3 Worker 命令壳切片已落盘；现 AIStor Free 方向已确认，正在核对精确版本和许可取得条件，不改变 P1–P5 整体完成标准。下面冻结当前精确树，不以占位代码冒充实现。
...
文档验证实际结果：8 份本片文档共 243 个本地链接目标全部存在（未验证段落锚点）；`git diff --check -- <上述文档>` 通过，仅有 LF/CRLF 提示。核对研究与行动 diff 后，拟只提交这两份文件的选型/许可检查点；其余同步文档混有开工前增量，留工作区，不整批纳入。提交前暂存区为空，未运行产品全量回归或双轴代码评审；本片无代码变更。
