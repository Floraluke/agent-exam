...
新增执行要求：每半小时检查 Codex 账户额度；若剩余 1–4%，先本地提交本目标安全检查点并报告停止。通过本机 Codex App Server 的 `account/rateLimits/read` 查询，只输出主窗口剩余百分比。最新检查为 **2026-09-18 00:40（新加坡时间）**：`PrimaryRemaining=15`，未触发停工阈值；下一次持续执行检查最迟 01:10。历史读数和早期检查方法失败保留在本文后续时间线；不以 goal token 数或另一限额池替代，不创建模型 turn、不读取/输出认证正文、不重置登录。
