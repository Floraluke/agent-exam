...
## ISSUE-01 GitHub 连通性不稳定：代理与直连会交替失效 —— 待处理
...
- **证据（2026-09-20 深夜复核）**：代理进程在运行且端口上有 ESTABLISHED 连接，但**新连接被拒**——`git fetch` 报 `Failed to connect to 127.0.0.1 port 7892 after 2111 ms`；同一时刻改走直连的 `git ls-remote` 与 `git push`（`ad609c5..73ad17e`）均成功。直连也有失败记录（21 秒超时）。两条路都会失效，因此这不是单向问题。
- **处理**：2026-09-20 深夜把仓库本地 `http.proxy` **移除**，默认走直连；实测连续两次 `git fetch upstream` 与一次 `git push origin lyq` 均成功。若直连再次超时而代理可用，用 `git config --local http.proxy http://127.0.0.1:7892` 加回即可（两种切换方式互相可逆）。
- **遗留风险**：两条路都不稳定——直连会 21 秒超时，代理会拒收新连接（原因未查明，重启代理软件可能恢复）。遇到 `port 443` 超时就开代理并写回配置，遇到 `port 7892` 拒绝就移除配置切回直连。**恢复克隆或换机器后必须重新确认这条配置。**
