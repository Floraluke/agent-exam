...
**原始制品（Raw Artifact）**：
一次评测运行产生的受限制过程证据。平台只向有权用户展示安全元数据，正文不作为公开证据下载；正文可按明确限额保留带标记的头尾版本，并在 `raw_30d` 到期后由所有者本机逐对象清理。
_Avoid_：公开证据、完整日志、自动过期删除

**核心证据（Core Evidence）**：
用于复现或确认确定性结论、不会被原始制品清理删除的长期证据，包括冻结配置、确定性结果、最终 patch 和公开测试摘要。
_Avoid_：全部制品、原始日志、临时缓存
