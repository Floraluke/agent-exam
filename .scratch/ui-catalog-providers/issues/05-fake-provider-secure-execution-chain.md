...
**Blocked by:** 项目负责人书面确认实施开工与 T1/T2 拆分；**T2 窗口**（固定 Harbor 是否允许替换其侧车网络附加只能在负责人机器回答）。安全合同三项已不再全部未决：额度上界与账本计数方式已由负责人 2026-09-21 拍板（A 保守上界）并已实现为候选；**请求字段白名单**仍须用固定 CLI 复核后才定稿。（2026-09-21 事实修正：本机**已安装 Docker**，T1 已在纯 Docker 层证成；原句"本机不安装 Docker"已失效。验收项本身未改动。）已有的 5/5 禁外网假令牌配置探针只作历史输入，不重复计为本项通过。
...
- **给 B 的契约请求（按任务单"E 侧提前告知、B 从契约侧配合"）**：登记受控 API 预设会在 **HTTP 响应序列化**阶段被拒——`AgentSummary` / `AgentDetail` 的 `model_provider: Literal["openai_chatgpt"]`（以及 `agent_type: Literal["codex"]`）是只含既有值的字面量集合，而 `POST /agent-configurations` 的 201 响应与列表项都要经它校验。因此这条链要走通，必须把该集合扩为**受控集合**（含受控假提供方值），否则注册本身就会失败。
