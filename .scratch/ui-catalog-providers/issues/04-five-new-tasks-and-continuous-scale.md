...
2. **规模侧范围收窄**（见左列对应条）：既有覆盖已包含大部分拒绝项，空白仅「0 个配置」；「重复 ID」的现有行为是**去重**，与早期草案措辞冲突，需先与 D 确认。
